...
func (tr *mwsTransporter) Dial(addr string, options ...DialOption) (conn net.Conn, err error) {
	...
	if !ok {
		...
		if opts.Chain == nil {
			...
			conn, err = opts.Chain.Dial(addr, FDProtectorChainOption(opts.FDProtector))
		}
		if err != nil {
			return
		}
...
func (tr *mwsTransporter) initSession(addr string, conn net.Conn, opts *HandshakeOptions) (*muxSession, error) {
	...
	if wsOptions.MuxMaxReceiveBuffer > 0 {
		smuxConfig.MaxReceiveBuffer = wsOptions.MuxMaxReceiveBuffer * 1024 * 1024
	}
	if wsOptions.MuxMaxStreamBuffer > 0 {
		smuxConfig.MaxStreamBuffer = wsOptions.MuxMaxStreamBuffer * 1024
	}
...
func MWSListener(addr string, options *WSOptions) (Listener, error) {
	...
	l := &mwsListener{
		...
		muxMaxReceiveBuffer: options.MuxMaxReceiveBuffer,
		muxMaxStreamBuffer:  options.MuxMaxStreamBuffer,
	}
...
func (l *mwsListener) mux(conn net.Conn) {
	smuxConfig := smux.DefaultConfig()
	if l.muxVersion > 0 {
		smuxConfig.Version = l.muxVersion
	}
	if l.muxMaxReceiveBuffer > 0 {
		smuxConfig.MaxReceiveBuffer = l.muxMaxReceiveBuffer * 1024 * 1024
	}
	if l.muxMaxStreamBuffer > 0 {
		smuxConfig.MaxStreamBuffer = l.muxMaxStreamBuffer * 1024
	}
