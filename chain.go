...
func makeControl(p FDProtector) func(string, string, syscall.RawConn) error {
	return func(network, address string, c syscall.RawConn) error {
		return c.Control(func(fd uintptr) {
			if !p.Protect(int32(fd)) {
				// TODO: Record and report these errors.
				log.Logf("[FDProtector] Failed to protect a %s socket", network)
			}
		})
	}
}
...
func makeDialer(p FDProtector, timeout time.Duration) (d *net.Dialer) {
	if p == nil {
		d = &net.Dialer{
			Timeout: timeout,
...
// DialWithFDProtector connects to the address on the named network using
...
// listener sockets.
func makeListenConfig(fdp FDProtector) *net.ListenConfig {
	if fdp == nil {
		return &net.ListenConfig{}
	}
	return &net.ListenConfig{
		Control: makeControl(fdp),
	}
}
...
func ListenWithFDProtector(network string, laddr *net.UDPAddr, fdp FDProtector) (*net.UDPConn, error) {
	c := makeListenConfig(fdp)
	if laddr == nil {
		laddr = &net.UDPAddr{}
	}
	conn, err := c.ListenPacket(context.Background(), network, laddr.String())
	if err != nil {
		return nil, err
	}
...
func (c *Chain) dialWithOptions(ctx context.Context, network, address string, options *ChainOptions) (net.Conn, error) {
	...
	if route.IsEmpty() {
		...
		case "udp", "udp4", "udp6":
			if address == "" {
				return ListenWithFDProtector(network, nil, options.FDProtector)
			}
		default:
		}
		return DialWithFDProtector(ctx, network, ipAddr, timeout, options.FDProtector)
	}
